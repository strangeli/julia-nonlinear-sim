...
# daily plotting
plot(2:num_days, load_amp_daily[2:num_days] , label = L"\bar P^{d,c}", linewidth=3, linestyle=:dot)
plot!(2:num_days, sum(mean_energy_d[2:num_days],dims=2) ./ 3600, label=L"\bar y^{c}", linewidth=3, linestyle=:dash)
plot!(2:num_days, sum(ILC_power_agg[2:num_days,1,:],dims=2), label=L"$\bar u^c$", ytickfontsize=14,
               xtickfontsize=18, margin=5Plots.mm,
