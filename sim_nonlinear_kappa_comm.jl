	...
	@everywhere begin
		dir = @__DIR__
		### CHANGE HourlyUpdateEcon to HourlyUpdate in system_structs before running this further!!!!!!!!!!!-
 ...
 #	Q = diagm(0=>ones(24))
	...
	### CHANGE HourlyUpdateEcon to HourlyUpdate in system_structs before (and re-include) running this further!!!!!!!!!!!
...
begin
	...
	using Dates
	date = Dates.now()
	CSV.write("$dir/data/$(date)_error_I_Q_lin.csv", DataFrame([[mean(norm_energy_d_I[i],dims=2) for i in 1:8][j][k] for j in 1:8, k in 1:20]'))
	# res = CSV.read("$dir/error.csv.csv")

	#######################################################

	using LaTeXStrings
	plot(mean(norm_energy_d_I[1],dims=2),legend=:right, label = L"\kappa = 0.00\, h^{-1}", ytickfontsize=14,
	           xtickfontsize=14, linestyle=:dot, margin=8Plots.mm, left_margin=15Plots.mm, ylims =(0,9900),
			   legendfontsize=14, linewidth=3,xaxis=("days [c]",font(14)), yaxis = ("2-norm of the error [s]",font(14))) #  ylims=(0,1e6)
	plot!(mean(norm_energy_d_I[2],dims=2), label= L"\kappa = 0.25\, h^{-1}", linewidth = 3, linestyle=:dashdotdot)
	plot!(mean(norm_energy_d_I[3],dims=2), label= L"\kappa = 0.50\, h^{-1}", linewidth = 3, linestyle=:dashdot)
	plot!(mean(norm_energy_d_I[4],dims=2),label=  L"\kappa = 0.75\, h^{-1}", linewidth = 3, linestyle=:dash)
	plot!(mean(norm_energy_d_I[5],dims=2), label= L"\kappa = 1.00\, h^{-1}", linewidth = 3, linestyle=:solid)
	#title!("Error norm for scenario I")
	savefig("$dir/plots/$(date)_kappa_Y6_homo_N_$(N)_I_Q_lin.png")

	using LaTeXStrings
	plot(mean(norm_energy_d_I[5],dims=2),legend=:right, label = L"\kappa = 1.00\, h^{-1}", ytickfontsize=14,
	...
	plot!(mean(norm_energy_d_I[6],dims=2),label=  L"\kappa = 1.25\, h^{-1}", linewidth = 3, linestyle=:dash)
	plot!(mean(norm_energy_d_I[7],dims=2),label=  L"\kappa = 1.50\, h^{-1}", linewidth = 3, linestyle=:dashdot)
	plot!(mean(norm_energy_d_I[8],dims=2),label=  L"\kappa = 1.75\, h^{-1}", linewidth = 3, linestyle=:dashdotdot)
	plot!(mean(norm_energy_d_I[9],dims=2), label= L"\kappa = 2.00\, h^{-1}", linewidth = 3, linestyle=:dot)
	#title!("Error norm for scenario I")
	savefig("$dir/plots/$(date)_kappa2_Y6_homo_N_$(N)_I_Q_lin.png")
end
	##############################################################################################
	###############################################################################################
	##############################################################################################
...
begin
	...
	using Dates
	date = Dates.now()
	CSV.write("$dir/data/$(date)_error_II_noQ_lin.csv", DataFrame([[mean(norm_energy_d_II[i],dims=2) for i in 1:8][j][k] for j in 1:8, k in 1:20]'))
	# res = CSV.read("$dir/error.csv.csv")
	...
	plot!(mean(norm_energy_d_II[2],dims=2), label= L"\kappa = 0.25\, h^{-1}", linewidth = 3, linestyle=:dashdotdot)
	plot!(mean(norm_energy_d_II[3],dims=2), label= L"\kappa = 0.5\, h^{-1}", linewidth = 3, linestyle=:dashdot)
	plot!(mean(norm_energy_d_II[4],dims=2),label=  L"\kappa = 0.75\, h^{-1}", linewidth = 3, linestyle=:dash)
	plot!(mean(norm_energy_d_II[5],dims=2), label= L"\kappa = 1\, h^{-1}", linewidth = 3, linestyle=:solid)
	title!("Error norm for scenario II")
	savefig("$dir/plots/$(date)_kappa_Y6_homo_N_$(N)_II_noQ_lin.png")

	using LaTeXStrings
	plot(mean(norm_energy_d_II[5],dims=2),legend=:topright, label = L"\kappa = 1\, h^{-1}", ytickfontsize=14,
	           xtickfontsize=14, linestyle =:solid, margin=8Plots.mm,left_margin=15Plots.mm, ylims =(0,9900),
			   legendfontsize=13, linewidth=3,xaxis=("days [c]",font(14)), yaxis=("2-norm of the error",font(14)))  # ylims=(0,1e6)
	plot!(mean(norm_energy_d_II[6],dims=2),label=  L"\kappa = 1.25\, h^{-1}", linewidth = 3, linestyle=:dash)
	plot!(mean(norm_energy_d_II[7],dims=2),label=  L"\kappa = 1.5\, h^{-1}", linewidth = 3, linestyle=:dashdot)
	plot!(mean(norm_energy_d_II[8],dims=2),label=  L"\kappa = 1.75\, h^{-1}", linewidth = 3, linestyle=:dashdotdot)
	#plot!(mean(norm_energy_d[9],dims=2), label= L"\kappa = 2 h^{-1}", linewidth = 3, linestyle=:dot)
	title!("Error norm for scenario II")
	savefig("$dir/plots/$(date)_kappa2_Y6_homo_N_$(N)_II_noQ_lin.png")
end
...
begin
	...
	using Dates
	date = Dates.now()
	CSV.write("$dir/data/$(date)_error_III_noQ_lin.csv", DataFrame([[mean(norm_energy_d_III[i],dims=2) for i in 1:8][j][k] for j in 1:8, k in 1:20]'))
	# res = CSV.read("$dir/error.csv.csv")
	...
	#######################################################
		...
		plot(mean(norm_energy_d_III[1],dims=2),legend=:topright, label = L"\kappa = 0\, h^{-1}", ytickfontsize=14,
		               xtickfontsize=14, linestyle=:dot, margin=8Plots.mm, left_margin=15Plots.mm, ylims =(0,9900),
		    		   legendfontsize=14, linewidth=3,xaxis=("days [c]",font(14)), yaxis = ("2-norm of the error",font(14))) #  ylims=(0,1e6)
		plot!(mean(norm_energy_d_III[2],dims=2), label= L"\kappa = 0.25\, h^{-1}", linewidth = 3, linestyle=:dashdotdot)
		plot!(mean(norm_energy_d_III[3],dims=2), label= L"\kappa = 0.5\, h^{-1}", linewidth = 3, linestyle=:dashdot)
		plot!(mean(norm_energy_d_III[4],dims=2),label=  L"\kappa = 0.75\, h^{-1}", linewidth = 3, linestyle=:dash)
		plot!(mean(norm_energy_d_III[5],dims=2), label= L"\kappa = 1\, h^{-1}", linewidth = 3, linestyle=:solid)
		title!("Error norm for scenario III")
		savefig("$dir/plots/$(date)_kappa_Y6_homo_N_$(N)_III_noQ_lin.png")

		using LaTeXStrings
		plot(mean(norm_energy_d_III[5],dims=2),legend=:topright, label = L"\kappa = 1\, h^{-1}", ytickfontsize=14,
		               xtickfontsize=14, linestyle =:solid, margin=8Plots.mm, left_margin=15Plots.mm, ylims =(0,9900),
		    		   legendfontsize=13, linewidth=3,xaxis=("days [c]",font(14)), yaxis=("2-norm of the error",font(14)))  # ylims=(0,1e6)
		plot!(mean(norm_energy_d_III[6],dims=2),label=  L"\kappa = 1.25\, h^{-1}", linewidth = 3, linestyle=:dash)
		plot!(mean(norm_energy_d_III[7],dims=2),label=  L"\kappa = 1.5\, h^{-1}", linewidth = 3, linestyle=:dashdot)
		plot!(mean(norm_energy_d_III[8],dims=2),label=  L"\kappa = 1.75\, h^{-1}", linewidth = 3, linestyle=:dashdotdot)
		#plot!(mean(norm_energy_d[9],dims=2), label= L"\kappa = 2 h^{-1}", linewidth = 3, linestyle=:dot)
		title!("Error norm for scenario III")
		savefig("$dir/plots/$(date)_kappa2_Y6_homo_N_$(N)_III_noQ_lin.png")
end
...
begin
		...
		@everywhere compound_pars = $_compound_pars
			...
			using Dates
			date = Dates.now()
			CSV.write("$dir/data/$(date)_error_IV_noQ_lin.csv", DataFrame([[mean(norm_energy_d_IV[i],dims=2) for i in 1:8][j][k] for j in 1:8, k in 1:20]'))
			# res = CSV.read("$dir/error.csv.csv")
		...
		#######################################################
				...
				plot(mean(norm_energy_d_IV[1],dims=2),legend=:topright, label = L"\kappa = 0\, h^{-1}", ytickfontsize=14,
				               xtickfontsize=14, linestyle=:dot, margin=8Plots.mm, left_margin=15Plots.mm, ylims =(0,9900),
				    		   legendfontsize=14, linewidth=3,xaxis=("days [c]",font(14)), yaxis = ("2-norm of the error",font(14))) #  ylims=(0,1e6)
				plot!(mean(norm_energy_d_IV[2],dims=2), label= L"\kappa = 0.25\, h^{-1}", linewidth = 3, linestyle=:dashdotdot)
				plot!(mean(norm_energy_d_IV[3],dims=2), label= L"\kappa = 0.5\, h^{-1}", linewidth = 3, linestyle=:dashdot)
				plot!(mean(norm_energy_d_IV[4],dims=2),label=  L"\kappa = 0.75\, h^{-1}", linewidth = 3, linestyle=:dash)
				plot!(mean(norm_energy_d_IV[5],dims=2), label= L"\kappa = 1\, h^{-1}", linewidth = 3, linestyle=:solid)
				title!("Error norm for scenario IV")
				savefig("$dir/plots/$(date)_kappa_Y6_homo_N_$(N)_IV_noQ_lin.png")

				using LaTeXStrings
				plot(mean(norm_energy_d_IV[5],dims=2),legend=:topright, label = L"\kappa = 1\, h^{-1}", ytickfontsize=14,
				               xtickfontsize=14, linestyle =:solid, margin=8Plots.mm, left_margin=15Plots.mm, ylims =(0,9900),
				    		   legendfontsize=13, linewidth=3,xaxis=("days [c]",font(14)), yaxis=("2-norm of the error",font(14)))  # ylims=(0,1e6)
				plot!(mean(norm_energy_d_IV[6],dims=2),label=  L"\kappa = 1.25\, h^{-1}", linewidth = 3, linestyle=:dash)
				plot!(mean(norm_energy_d_IV[7],dims=2),label=  L"\kappa = 1.5\, h^{-1}", linewidth = 3, linestyle=:dashdot)
				plot!(mean(norm_energy_d_IV[8],dims=2),label=  L"\kappa = 1.75\, h^{-1}", linewidth = 3, linestyle=:dashdotdot)
				#plot!(mean(norm_energy_d[9],dims=2), label= L"\kappa = 2 h^{-1}", linewidth = 3, linestyle=:dot)
				title!("Error norm for scenario IV")
				savefig("$dir/plots/$(date)_kappa2_Y6_homo_N_$(N)_IV_noQ_lin.png")
end
